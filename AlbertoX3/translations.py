...
from interactions import Absent, BaseContext, MISSING
...
def language_wrapper(func: Callable[P, Awaitable[T]]) -> Callable[P, Awaitable[T]]:
    ...
    async def decorator(*args: P.args, **kwargs: P.kwargs) -> T:
        ctx: BaseContext | MISSING

        # check kwargs for "ctx", if not present check args
        if not isinstance(ctx := kwargs.get("ctx", MISSING), BaseContext):
            for arg in args:
...
class TranslationNamespace:
    ...
    def tn_add_source(self, source: Path) -> None:
...
class Translations:
    ...
    def register_translation_namespace(self, name: str, file: Path) -> None:
