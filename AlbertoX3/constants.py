...
class Config:
    ...
    def __new__(cls, path: Path):
        ...
        cls.TMP_FOLDER_RAW = cast(str, folder := tmp.get("folder", "tmp"))
        folder = Path(folder)  # type: ignore
        if not folder.is_absolute():
            folder = path.parent.joinpath(folder)
        cls.TMP_FOLDER = folder
        cls.TMP_PATTERN = FormatStr(tmp.get("pattern", "{extension}.{id}.alberto-x3.tmp"))
        cls.TMP_REMOVE_AUTO = get_bool(cast(dict, tmp.get("remove", {})).get("auto", True))
        cls.TMP_REMOVE_ON_STARTUP = get_bool(cast(dict, tmp.get("remove", {})).get("on_startup", True))

        # auto  # ToDo: make configurable
        cls.AUTO_HOUR = 0
        cls.AUTO_MINUTE = 0
        cls.AUTO_SECOND = 0
